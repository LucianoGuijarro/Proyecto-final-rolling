...
const Cards = ({ juego, flag, setFlag }) => {
  ...
  const confirmarDestacado = () => {
    clientAxios
      .patch(`/juegos/editarJuego/${juego._id}`, {
  ...
  const handleForm = (e) => {
    e.preventDefault();
    clientAxios
      .patch(`/juegos/editarJuego/${juego._id}`, infoEditada)
      .then((response) => {
        if (response.status === 200) {
          Swal.fire({
            icon: "success",
  ...
  const eliminarProducto = () => {
    if (window.confirm("Estas seguro de querer eliminar este juego?")) {
      clientAxios
        .delete(`/juegos/eliminarJuego/${juego._id}`)
        .then((response) => {
          if (response.status !== 200) {
            Swal.fire({
              icon: "error",
  ...
  return (
    <>
      <div
        className={`col-sm-7 col-md-3 col-lg-2 d-flex flex-wrap justify-content-center me-1 mt-5 p-0`}
      >
        <Link to={`/verJuego/${juego._id}`}>
          <img
            className={`${styles.img} ${styles.card} rounded-top img-fluid`}
            src={juego.portada}
            alt={juego.nombre}
          />
          <div className="d-flex justify-content-around w-100">
            <button
              className={`${styles.boton} rounded-bottom w-100 opacity-75`}
            >
              <TiShoppingCart size={35} color="white" />
            </button>
          </div>
        </Link>
        {rolUsuario === "admin" ? (
          <div className="col-sm-12 col d-flex justify-content-center">
            <button
              ...
              className={`${styles.botonEliminar} me-4`}
            >
            ...
            <button
              ...
              data-bs-target={`#modal-${juego._id}`}
              className={`${styles.botonEditar}`}
            >
              <FiEdit color="white" size={28} />
            </button>
